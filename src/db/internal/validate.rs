...
pub fn validate_username(username: &str) -> Result<(), PPError> {
    ...
    let numbers: Vec<char> = (1..9).map(|e| e.into()).collect();

    let mut allowed_characters: Vec<char> = lowercase
        .iter()
    ...
    if !username_body.chars().all(|c| allowed_set.contains(&c)) {
        return Err(PPError::from(
            "Invalid Username characters provided! Allowed are: a..z, A..Z, _",
        ));
    }
